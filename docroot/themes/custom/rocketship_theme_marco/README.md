# Rocketship theme: Rocketship Marco
...
rocketship_theme_marco.theme. You don't need to touch these
...
The aliases used with these includes, are defined in rocketship_theme_marco.theme. The
...
- `rocketship_theme_marco.layouts.yml`: The regions are defined here for custom layouts
...
This can be done by attaching a library via the theme's `rocketship_theme_marco.theme` file's `HOOK_preprocess_page` or in a Twig template, for example.
...
This is done by attaching a library via the theme's `rocketship_theme_marco.theme` file's `HOOK_preprocess_page`
